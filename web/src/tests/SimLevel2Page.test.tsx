...
describe("SimLevel2Page", () => {
  ...
  it("uses localhost domain for start level fetch", async () => {
    ...
    render(<SimLevel2Page />);

    // Click the "Start Level 2" button
    fireEvent.click(screen.getByRole("button", { name: /start level 2/i }));

    // Wait for the fetch to be called
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith(
        "http://localhost:5000/api/sim/level2/start",
