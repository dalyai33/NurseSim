...
export const SimLevel2Page: React.FC = () => {
  ...
  async function startOrResumeLevel2(retake = false){
    ...
    try{
      ...
      if(!res.ok) throw new Error("Failed to start level 2 attempt: " + res.statusText);
  ...
  async function submitAnswer(selectedIndex: number){
    ...
    try{
      ...
      if(data.game_over){
        ...
        setPopupMode("done");
        setPopupTitle("Level 2 Complete 🎉");
      }
      ...
      >
        ...
        {!showIntro && (
          <div className="quiz-popup">
            ...
            {!loading && !errorMsg && !step && (
              <div>
                <h1 className="sim-coming-soon-title">Level 2 Complete 🎉</h1>
                ...
                {scorePercent !== null ? (
                  <>
                    ...
                    <button className="close-button" onClick={()=> startOrResumeLevel2(true)}>
                      Retake Level 2
                    </button>
                  </>
                ):(
